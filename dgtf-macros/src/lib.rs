...
struct SpriteSheet {
    pixels_per_byte: u8, // implies palette size
    palette: [u8; 16],
    width: u8,
    height: u8,
    pixel_array: &'static[u8]
    // : &'static[u8]
}
...
pub fn include_spritesheet(input: TokenStream) -> TokenStream {
    // Convert input to a string literal (assuming input is a string literal)
    let file_path = input.to_string().trim_matches('"').to_string();
    ...
    //     pixels_per_byte,
    //     palette,
    //     width: header.image_size.width as u8,
    //     height: header.image_size.height as u8,
    ...
    let output = quote! {
        {
            ...
            struct SpriteSheet {
                ...
                width: u8,
                height: u8,
                pixel_array: [u8; #pixel_array_size], // pixel_array_size is determined dynamically
            }
